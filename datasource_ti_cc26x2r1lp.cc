...
void kis_datasource_ti_cc26x2r1lp::handle_rx_packet(kis_packet *packet) {
    typedef struct {
    ...
    auto ti_cc26x2r1lp_chunk = packet->fetch<kis_datachunk>(pack_comp_linkframe);
    ...
    if (ti_cc26x2r1lp_chunk->length < 10) {
        // fmt::print(stderr, "debug - ti_cc26x2r1lp too short ({} < 10)\n",
        // ti_cc26x2r1lp_chunk->length);
        delete (packet);
        return;
    }

printf("kis_datasource_ti_cc26x2r1lp\n");

for(int re=0;re<ti_cc26x2r1lp_chunk->length;re++)
    printf("%02X",ti_cc26x2r1lp_chunk->data[re]);
printf("\n");


//we may have to redo the checksum...
/**
    if (!checksum(ti_cc26x2r1lp_chunk->data, ti_cc26x2r1lp_chunk->length)) {
        delete (packet);
        return;
    }
**/
/**
    ...
    if (ti_cc26x2r1lp_chunk->data[0] == 0x02 && ti_cc26x2r1lp_chunk->data[1] == 0x86 &&
        ti_cc26x2r1lp_chunk->data[2] == 0x03) {
/**/
        if(ti_cc26x2r1lp_chunk->data[(ti_cc26x2r1lp_chunk->length-3)] = 0x80) {
            uint32_t rssi = ti_cc26x2r1lp_chunk->data[(ti_cc26x2r1lp_chunk->length-4)];
            uint16_t ti_cc26x2r1lp_payload_len = ti_cc26x2r1lp_chunk->data[11];
            uint8_t channel = ti_cc26x2r1lp_chunk->data[4];
            // We can make a valid payload from this much
            auto conv_buf_len = sizeof(_802_15_4_tap) + ti_cc26x2r1lp_payload_len;
            ...
            memcpy(conv_header->payload, &ti_cc26x2r1lp_chunk->data[12], ti_cc26x2r1lp_payload_len);
            ...
            ti_cc26x2r1lp_chunk->set_data((uint8_t *) conv_header, conv_buf_len, false);
            ti_cc26x2r1lp_chunk->dlt = KDLT_IEEE802_15_4_TAP;
...
/**
    } else if (ti_cc26x2r1lp_chunk->data[0] == 0x02 && ti_cc26x2r1lp_chunk->data[1] == 0x4E &&
               ti_cc26x2r1lp_chunk->data[2] == 0x7F) {
        // Convert the channel for the btlell header
        auto bt_channel = ti_cc26x2r1lp_chunk->data[5];
        uint8_t channel = ti_cc26x2r1lp_chunk->data[5];
        ...
        unsigned int ti_cc26x2r1lp_payload_len =
            ti_cc26x2r1lp_chunk->length - 13;  // minus header and checksum
        // We can make a valid payload from this much
        auto conv_buf_len = sizeof(btle_rf) + ti_cc26x2r1lp_payload_len;
        btle_rf *conv_header =
        ...
        // RSSI not sure yet
        conv_header->signal = ti_cc26x2r1lp_chunk->data[6];

        uint16_t bits = btle_rf_crc_checked;
        // if (true)//not sure yet
        //    bits += btle_rf_crc_valid;

        if (ti_cc26x2r1lp_payload_len >= 4) {
        ...
        ti_cc26x2r1lp_chunk->set_data((uint8_t *) conv_header, conv_buf_len, false);
        ti_cc26x2r1lp_chunk->dlt = KDLT_BTLE_RADIO;
        ...
        decapchunk->set_data(conv_header->payload, ti_cc26x2r1lp_payload_len, false);
