...
int atzb_enter_promisc_mode(kis_capture_handler_t *caph, uint8_t chan) {
    ...
    uint8_t cmd_1[5] = {0x01, 0x02, 0x02, 0x04, 0x04};//01 02 02 04 04 
    uint8_t rep_1[6] = {0x01, 0x03, 0x02, 0x04, 0x39, 0x04};//res:6 01 03 02 04 39 04 
    res = atzb_write_cmd(caph, cmd_1, 5, rep_1, 6, NULL, 0);
    if (res < 0)
        return res;

    uint8_t cmd_2[5] = {0x01, 0x02, 0x02, 0x04, 0x04};//01 02 02 04 04 
    uint8_t rep_2[6] = {0x01, 0x03, 0x02, 0x04, 0x39, 0x04};//res:6 01 03 02 04 39 04 
    res = atzb_write_cmd(caph, cmd_2, 5, rep_2, 6, NULL, 0);
    if (res < 0)
        return res;

    uint8_t cmd_3[5] = {0x01, 0x02, 0x02, 0x0B, 0x04};//01 02 02 0B 04
    uint8_t rep_3[6] = {0x01, 0x03, 0x02, 0x0B, 0x07, 0x04};//res:6 01 03 02 0B 07 04 
    res = atzb_write_cmd(caph, cmd_3, 5, rep_3, 6, NULL, 0);
    if (res < 0)
        return res;

    uint8_t cmd_4[5] = {0x01, 0x02, 0x02, 0x0C, 0x04};//01 02 02 0C 04
    uint8_t rep_4[6] = {0x01, 0x03, 0x02, 0x0C, 0x22, 0x04};//res:6  01 03 02 0C 22 04 
    res = atzb_write_cmd(caph, cmd_4, 5, rep_4, 6, NULL, 0);
    if (res < 0)
        return res;

    uint8_t cmd_5[5] = {0x01, 0x02, 0x02, 0x03, 0x04};//01 02 02 03 04 
    uint8_t rep_5[9] = {0x01, 0x06, 0x02, 0x03, 0xC8, 0x00, 0x00, 0x00, 0x04};//res:9  01 06 02 03 C8 00 00 00 04 
    res = atzb_write_cmd(caph, cmd_5, 5, rep_5, 9, NULL, 0);
    if (res < 0)
        return res;

    uint8_t cmd_6[6] = {0x01, 0x03, 0x02, 0x01, 0x0A, 0x04};//01 03 02 01 0A 04
    cmd_6[4] = chan;//set the chan
    uint8_t rep_6[6] = {0x01, 0x03, 0x02, 0x01, 0x01, 0x04};// res:6 01 03 02 01 01 04 
    res = atzb_write_cmd(caph, cmd_6, 6, rep_6, 6, NULL, 0);
    if (res < 0)
        return res;

    uint8_t cmd_7[6] = {0x01, 0x03, 0x02, 0x02, 0x02, 0x04};//01 03 02 02 02 04
    uint8_t rep_7[6] = {0x01, 0x03, 0x02, 0x02, 0x01, 0x04};// 01 03 02 02 01 04 
    res = atzb_write_cmd(caph, cmd_7, 6, rep_7, 6, NULL, 0);
    if (res < 0)
        return res;

    uint8_t cmd_8[5] = {0x01, 0x02, 0x02, 0x05, 0x04};//01 02 02 05 04  
    uint8_t rep_8[6] = {0x01, 0x03, 0x02, 0x05, 0x01, 0x04};//res:6 01 03 02 05 01 04 
    res = atzb_write_cmd(caph, cmd_8, 5, rep_8, 6, NULL, 0);
    if (res < 0)
        return res;
...
int probe_callback(kis_capture_handler_t *caph, uint32_t seqno,
    ...
    *ret_interface = cf_params_interface_new();
    ...
    if (strstr(interface, "atzb_x_212b_usb") != interface) {
        free(interface);
        return 0;
    }
    ...
    snprintf(cap_if, 32, "atzb_x_212b_usb-%012X", adler32_csum((unsigned char *) device, strlen(device)));
    ...
    if ((placeholder_len = cf_find_flag(&placeholder, "uuid", definition)) > 0) {
        *uuid = strndup(placeholder, placeholder_len);
        ...
        snprintf(errstr, STATUS_MAX, "%08X-0000-0000-0000-%012X",
                adler32_csum((unsigned char *) "kismet_cap_atzb_x_212b_usb",
                    strlen("kismet_cap_atzb_x_212b_usb")) & 0xFFFFFFFF,
                adler32_csum((unsigned char *) device, strlen(device)));
        *uuid = strdup(errstr);
    }

    (*ret_interface)->capif = strdup(cap_if);
    (*ret_interface)->hardware = strdup("atzb_x_212b_usb");

    /* atzb_x_212b_usb supports 1-10 for zigbee */
    char chstr[4];
    int ctr = 0;

    (*ret_interface)->channels = (char **) malloc(sizeof(char *) * 10);
        ...
        *localchan = atoi(localchanstr); 
        ...
        if (localchan == NULL) {
            snprintf(msg, STATUS_MAX,
                    "atzb x 212b usb could not parse channel= option provided in source "
                    "definition");
            return -1;
        }
    } else {
...
void *chantranslate_callback(kis_capture_handler_t *caph, char *chanstr) {
    ...
    if (sscanf(chanstr, "%u", &parsechan) != 1) {
        snprintf(errstr, STATUS_MAX, "1 unable to parse requested channel '%s'; atzb_x_212b_usb channels "
                "are from 1 to 10", chanstr);
        cf_send_message(caph, errstr, MSGFLAG_INFO);
        return NULL;
    }

    if (parsechan > 10 || parsechan < 1) {
        snprintf(errstr, STATUS_MAX, "2 unable to parse requested channel '%u'; atzb_x_212b_usb channels "
                "are from 1 to 10", parsechan);
        cf_send_message(caph, errstr, MSGFLAG_INFO);
        return NULL;
    }
...
int main(int argc, char *argv[]) {
    ...
    kis_capture_handler_t *caph = cf_handler_init("atzb_x_212b_usb");
